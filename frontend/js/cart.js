...
async function fetchAndRenderCartItemList() {
    ...
    try {
        ...
        if (result.code === 1000) {
            ...
            cartTotal.textContent = `${cart.total} VNĐ`;
        } else {
...
function createCartItemRow(cartItem) {
    ...
    productPriceP.textContent = cartItem.price + ' VNĐ';
    ...
    subButton.onclick = (event) => decreaseQuantity(cartItem.cartItemId);
    ...
    addButton.onclick = (event) => increaseQuantity(cartItem.cartItemId, cartItem.product.stockQuantity);

    totalP.textContent = `${cartItem.price * cartItem.quantity} VNĐ`;
...
async function updateQuantity(cartItemId) {
    ...
    try {
        ...
        if (result.code === 1000) {
            ...
            document.getElementById(`totalP_${cartItemId}`).textContent = `${kq.quantity * kq.price} VNĐ`;
            cartTotal.textContent = kq.cartTotal + ' VNĐ';
        } else {
            switch (response.status) {
                case 400:
                    messageDiv.textContent = result.message;
